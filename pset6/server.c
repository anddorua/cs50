...
char* indexes(const char* path)
{
    ...
    int i = 0;
    while(index_files[i][0] != '\0') {
        strcpy(buf + path_len, index_files[i]);
        if (access(buf, 0) == 0) {
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    size_t read = 0;
    size_t pos_to_read = 0;
    
    while (!feof(file)) {
        size_t c_read = fread(buf + pos_to_read, 1, buf_length - pos_to_read, file);
        read += c_read;
        if (c_read != (buf_length - pos_to_read)) {
            // eof occured
            (*content) = malloc(read);
            memcpy(*content, buf, read);
            *length = read;
            free(buf);
            return true;
        }
        ...
        char* new_buf = malloc(buf_length);
        memcpy(new_buf, buf, read);
        free(buf);
        buf = new_buf;
        pos_to_read = read;
    }
...
const char* lookup(const char* path)
{
    char buf [LimitRequestLine];
    int len = strlen(path);
    int i = 0;
    for (; i < len && i < LimitRequestLine; i++) {
    ...
    i = 0;
    while((*(mimes[i].ext)) != '\0') {
        if (strcmp(dot_pos, mimes[i].ext) == 0) {
            return mimes[i].mime;
        }
        i++;
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* http_version = strchr(request_target, ' ');
    if (!http_version) {
        error(400); // http version not found
        return false;
    }
    
    if (strncmp(http_version + 1, "HTTP/1.1", 8) != 0) {
        error(505); // http version not supported
        return false;
    }
    char buf [LimitRequestLine];
    strncpy(buf, request_target, http_version - request_target);
    buf[http_version - request_target] = '\0';
    ...
    if (separator) {
        strncpy(abs_path, buf, separator - buf);
        abs_path[separator - buf] = '\0';
        strcpy(query, separator + 1);
    } else {
        strcpy(abs_path, buf);
        query[0] = '\0';
    }
